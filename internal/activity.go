...
func WithActivityTask(
	...
	logger = log.With(logger,
		tagActivityID, task.GetActivityId(),
		tagActivityType, task.GetActivityType().GetName(),
	...
	return newActivityContext(ctx, interceptors, &activityEnvironment{
		...
		activityType:   ActivityType{Name: task.GetActivityType().GetName()},
		...
		heartbeatDetails:       task.GetHeartbeatDetails(),
		workflowType: &WorkflowType{
			Name: task.GetWorkflowType().GetName(),
		},
		workflowNamespace:  task.GetWorkflowNamespace(),
		retryPolicy:        convertFromPBRetryPolicy(task.GetRetryPolicy()),
